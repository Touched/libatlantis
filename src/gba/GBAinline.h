...
unreadable:
  ...
  atlantis_debug_read_word(address, value);
...
unreadable:
  ...
  atlantis_debug_read_halfword(address, value);
...
static inline s16 CPUReadHalfWordSigned(u32 address)
{
  ...
  atlantis_debug_read_signed_halfword(address, value);
  return (s16)value;
}
...
static inline u8 CPUReadByte(u32 address)
{
...
#endif
        atlantis_debug_read_byte(address, biosProtected[address & 3]);
        return biosProtected[address & 3];
      } else goto unreadable;
    }
    atlantis_debug_read_byte(address, bios[address & 0x3FFF]);
    return bios[address & 0x3FFF];
  case 2:
	atlantis_debug_read_byte(address, workRAM[address & 0x3FFFF]);
    return workRAM[address & 0x3FFFF];
  case 3:
	atlantis_debug_read_byte(address, internalRAM[address & 0x7fff]);
    return internalRAM[address & 0x7fff];
  case 4:
    if((address < 0x4000400) && ioReadable[address & 0x3ff]) {
      atlantis_debug_read_byte(address, ioMem[address & 0x3ff]);
      return ioMem[address & 0x3ff];
    }
    else goto unreadable;
  case 5:
	atlantis_debug_read_byte(address, paletteRAM[address & 0x3ff]);
  ...
  case 6:
    ...
    if ((address & 0x18000) == 0x18000)
      address &= 0x17fff;
    atlantis_debug_read_byte(address, vram[address]);
    return vram[address];
  case 7:
	atlantis_debug_read_byte(address, oam[address & 0x3ff]);
  ...
  case 11:
  case 12:
	atlantis_debug_read_byte(address, rom[address & 0x1FFFFFF]);
    return rom[address & 0x1FFFFFF];
  case 13:
	atlantis_debug_read_byte(address, eepromRead(address));
  ...
  case 15:
  {
	if (cpuEEPROMSensorEnabled) {
		...
		case 0x8200:
			atlantis_debug_read_byte(address, systemGetSensorX() & 255);
			return systemGetSensorX() & 255;
		case 0x8300:
			atlantis_debug_read_byte(address, (systemGetSensorX() >> 8) | 0x80);
			return (systemGetSensorX() >> 8) | 0x80;
		case 0x8400:
			atlantis_debug_read_byte(address, systemGetSensorY() & 255);
			return systemGetSensorY() & 255;
		case 0x8500:
			atlantis_debug_read_byte(address, systemGetSensorY() >> 8);
			return systemGetSensorY() >> 8;
		}
	}
	atlantis_debug_read_byte(address, flashRead(address));
...
#endif
	if(cpuDmaHack) {
		atlantis_debug_read_byte(address, cpuDmaLast & 0xFF);
		return cpuDmaLast & 0xFF;
	} else {
		if(armState) {
			atlantis_debug_read_byte(address, CPUReadByteQuick(reg[15].I + (address & 3)));
			return CPUReadByteQuick(reg[15].I + (address & 3));
		} else {
			atlantis_debug_read_byte(address, CPUReadByteQuick(reg[15].I + (address & 1)));
...
{
	atlantis_debug_write_word(address, value);
#ifdef GBA_LOGGING
  if(address & 3) {
    if(systemVerbose & VERBOSE_UNALIGNED_MEMORY) {
...
{
	atlantis_debug_write_halfword(address, value);
#ifdef GBA_LOGGING
  if(address & 1) {
    if(systemVerbose & VERBOSE_UNALIGNED_MEMORY) {
...
{
  atlantis_debug_write_byte(address, b);
  switch(address >> 24) {
  case 2:
#ifdef BKPT_SUPPORT
